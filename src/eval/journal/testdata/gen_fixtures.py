...
def run_fixtures():
    ...
    return [
        ...
        ("m2_excerpt_session_start_no_shape", m2,
         lx_del(lx_del(log, 0, "config_profile"), 0, "loaded_extensions"), M2_SEL,
         ("Association", "event:0", "session_start.shape")),
...
# -- P1.6: the normalized configuration (A9b's comparator) --------------------
#
# ADR-004 D2/D3 A9b, PLAN-004 §3 P1.6's provenance table: the manifest's
# `normalized_configuration` is the canonical JSON of the reader's
# RecordedConfig ++ T0Settings; its independent check is THIS config reader,
# over the same journal and host log, and the entry's T0 table. Keys sorted,
# compact, non-ASCII verbatim. Written from D1's settings table, not from
# `src/eval/journal/admission.ail`.

ADMISSION_OUT = Path(__file__).with_name("admission_fixtures.ail")

UNSERVED_ENV = ["MOTOKO_CONFIG", "MOTOKO_REPO", "MOTOKO_MODELS_FILE"]


def normalized_configuration(jl, lg, s, env):
    w = read_world(jl, lg, s, env)
    header = read(jl, s)["path"][0]
    boot = header["boot"]
    enc = w["encoding"]
    config = {
        "api_model": w["api"],
        "checkpoint_enabled": False,
        "context_limit": f"{enc[0]}:{enc[1]}",
        "env": [[k, v] for k, v in w["env"]],
        "ext_set_digest": header["ext_set_digest"],
        "files": [[PROFILE_CONFIG, enc[2]]],
        "header_model": header["model"],
        "hybrid_predicate": w["predicate"],
        "hybrid_tools": boot["hybrid_tools"],
        "logical_model": w["logical"],
        "max_cost_millicents": 0,
        "ohmy_pi": boot["ohmy_pi"],
        "profile": header["profile"],
        "profile_dir": EVAL_PROFILE_DIR,
        "session_id": header["session_id"],
        "system_prefix_digest": header["system_prefix_digest"],
        "unserved_env": list(UNSERVED_ENV),
...
def admission_fixtures():
    ...
    for name, jl, lg, s, expected, env in world_fixtures():
        if expected is None:
            out.append((name, normalized_configuration(jl, lg, s, env)))
    return out
...
def render_admission():
    lines = [
        ...
        "-- `--check` fails when this file is stale.",
        "--",
        "-- P1.6 (A9b): per admitted world fixture, the normalized configuration",
        "-- computed by the generator's own config reader.",
        "",
        "module src/eval/journal/testdata/admission_fixtures",
        "",
        "export type AdmissionFixture = { name: string, normalized_configuration: string }",
        "",
        "export pure func fxa_all() -> [AdmissionFixture] {",
        "  [",
    ]
    rows = admission_fixtures()
    for i, (name, nc) in enumerate(rows):
        sep = "," if i + 1 < len(rows) else ""
        lines.append(f"    {{ name: {lit(name)}, normalized_configuration: {lit(nc)} }}{sep}")
    lines += ["  ]", "}", ""]
    return "\n".join(lines)


# -- P1.7a: the M15 A-row near-misses, each with its LIVE excerpt ------------
#
# ADR-004 D3 A1–A4, PLAN-004 §3 P1.7a and M15. The family's part (P1.2a,
# P1.2b) landed each near-miss journal. A near-miss models a snapshot whose
# content changed AFTER the live run wrote its host log, so the live excerpt
# is the log of the UNALTERED twin — `build_log` over the altered journal
# would agree with the alteration and nothing could catch it. The one
# exception is `m15_association`, whose alteration IS the excerpt (P1.2b's
# fixture pairs the clean journal with that altered log; it is used as is).
#
# The expectations are this generator's own:
#   * `prefix_seq` / `prefix_digest`: the last `history_appended` or
#     `history_replaced` entry on the path before `start`, and its RECORDED
#     `digest_after` (A1's comparand);
#   * `append_seqs` / `append_digests`: the kept segment's appends and their
#     recorded `digest_after` (A1, A2);
#   * `live_payload`: the selected run's `provider_call_prepared.payload_digest`
#     per associated call, from this file's excerpt reader;
#   * `replay_payload`: the payload digest of the request the journal implies
#     before each kept call (the fold before it) — what a replay that serves
#     the journal's content sends;
#   * `first_request_divergence`: the first call k (1-based) whose two digests
#     differ, 0 if none. It is A3's expected position ONLY where the replay
#     reproduces the journal before call k (A2 passes); the swap row is not
#     such a row (the loop's correlation guard answers there, which this file
#     does not model) and its expectation is pinned in MATRIX.expected.tsv.

SOURCE_OUT = Path(__file__).with_name("source_check_fixtures.ail")


def near_miss_pairs():
    base = chain(base_bodies())
    base_log = build_log(base, "e23")
    m2 = chain(m2_bodies())
    m2_log = build_log(m2, "e24")
    jf = {name: lines for name, lines, _, _ in journal_fixtures()}
    rf = {name: (jl, lg) for name, jl, lg, _, _ in run_fixtures()}
    return [
        ("clean_base", "fxj_clean", base, base_log, BASE_SEL),
        ("near_miss_chain_break", "fxj_near_miss_chain_break", jf["near_miss_chain_break"], base_log, BASE_SEL),
        ("near_miss_malformed_entry", "fxj_near_miss_malformed_entry", jf["near_miss_malformed_entry"],
         base_log, BASE_SEL),
        ("clean_m2", "fxr_m2_clean", m2, m2_log, M2_SEL),
        ("m15_unexpected_tool_result", "fxr_m15_unexpected_tool_result", rf["m15_unexpected_tool_result"][0],
         m2_log, M2_SEL),
        ("m15_malformed_tool_arguments", "fxr_m15_malformed_tool_arguments",
         rf["m15_malformed_tool_arguments"][0], m2_log, M2_SEL),
        ("m15_duplicate_call_id_swap", "fxr_m15_duplicate_call_id_swap", rf["m15_duplicate_call_id_swap"][0],
         m2_log, M2_SEL),
        ("m15_association", "fxr_m15_association", rf["m15_association"][0], rf["m15_association"][1], M2_SEL),
    ]


def source_expectations(jl, lg, s):
    r = read(jl, s)
    path = r["path"]
    si = [e["id"] for e in path].index(s["start"])
    prefix = [e for e in path[:si] if e["type"] in ("history_appended", "history_replaced")][-1]
    seg = kept_segment(jl, s)
    appends = [e for e in seg if e["type"] == "history_appended"]
    a = read_run(jl, log_text(lg), s)
    events = {e["index"]: e for e in a["events"]}
    live = [events[c["prepared"]]["payload_digest"] for c in a["calls"]]
    by_seq = {e["seq"]: e for e in path}
    replay = [payload_digest(fold(jl, by_seq[c["seq"]]["parent_id"])["history"]) for c in a["calls"]]
    first = next((k for k, (x, y) in enumerate(zip(live, replay), 1) if x != y), 0)
    return {
        "prefix_seq": prefix["seq"], "prefix_digest": prefix["digest_after"],
        "append_seqs": [e["seq"] for e in appends], "append_digests": [e["digest_after"] for e in appends],
        "call_seqs": [c["seq"] for c in a["calls"]],
        "normalized_configuration": normalized_configuration(jl, lg, s, ENV_DEFAULT),
        "live_payload": live, "replay_payload": replay, "first_request_divergence": first,
    }


def render_source_checks():
    lines = [
        "-- GENERATED by src/eval/journal/testdata/gen_fixtures.py -- do not edit.",
        "-- Regenerate with `python3 src/eval/journal/testdata/gen_fixtures.py`;",
        "-- `--check` fails when this file is stale.",
        "--",
        "-- P1.7a (A1–A4, M15's A-rows): each near-miss journal the family's part",
        "-- landed, paired with the LIVE excerpt (the host log of its unaltered",
        "-- twin), and this generator's own expectations: the recorded prefix",
        "-- digest, the kept appends' recorded digests, the first call whose",
        "-- live payload digest differs from the one the journal implies, and the",
        "-- normalized configuration (A9b's comparator, as for the world fixtures).",
        "-- `family` names the family's fixture whose journal this is.",
        "",
        "module src/eval/journal/testdata/source_check_fixtures",
        "",
        "export type SourceCheckFixture = {",
        "  name: string, family: string, journal: [string], log: [string],",
        "  leaf: string, run_id: string, start: string, end_kind: string, end_id: string,",
        "  prefix_seq: int, prefix_digest: string, append_seqs: [int], append_digests: [string],",
        "  call_seqs: [int], live_payload: [string], replay_payload: [string], first_request_divergence: int,",
        "  normalized_configuration: string",
        "}",
        "",
    ]
    names = []
    for name, family, jl, lg, s in near_miss_pairs():
        x = source_expectations(jl, lg, s)
        names.append(name)
        journal = [json.dumps(e, ensure_ascii=False, separators=(",", ":")) for e in jl]
        fields = [
            f"name: {lit(name)}, family: {lit(family)}",
            f"journal: {ail_list([lit(t) for t in journal], 6)}",
            f"log: {ail_list([lit(t) for t in log_text(lg)], 6)}",
            f"leaf: {lit(s['leaf'])}, run_id: {lit(s['run_id'])}, start: {lit(s['start'])}, "
            f"end_kind: {lit(s['end'][0])}, end_id: {lit(s['end'][1])}",
            f"prefix_seq: {x['prefix_seq']}, prefix_digest: {lit(x['prefix_digest'])}",
            f"append_seqs: {ail_ints(x['append_seqs'])}, append_digests: {ail_strings(x['append_digests'])}",
            f"call_seqs: {ail_ints(x['call_seqs'])}, live_payload: {ail_strings(x['live_payload'])}",
            f"replay_payload: {ail_strings(x['replay_payload'])}, "
            f"first_request_divergence: {x['first_request_divergence']}",
            f"normalized_configuration: {lit(x['normalized_configuration'])}",
        ]
        lines += [f"export pure func fxs_{name}() -> SourceCheckFixture {{", "  { " + ",\n      ".join(fields) + " }",
                  "}", ""]
    lines += ["export pure func fxs_all() -> [SourceCheckFixture] {",
              f"  {ail_list([f'fxs_{n}()' for n in names], 2)}", "}", ""]
    return "\n".join(lines)


def print_normalized_configuration(name):
    for fname, jl, lg, s, expected, env in world_fixtures():
        if fname == name:
            if expected is not None:
                print(f"fixture {name} is refused by the reader", file=sys.stderr)
                return 1
...
def main(argv):
    outputs = [(OUT, render()), (JOURNAL_OUT, render_journal()), (ASSOC_OUT, render_association()),
               (WORLD_OUT, render_world()), (ADMISSION_OUT, render_admission()),
               (SOURCE_OUT, render_source_checks())]
    if "--normalized-configuration" in argv:
        i = argv.index("--normalized-configuration")
        if i + 1 >= len(argv):
            print("usage: gen_fixtures.py --normalized-configuration <world fixture>", file=sys.stderr)
            return 2
        return print_normalized_configuration(argv[i + 1])
    if "--check" in argv:
