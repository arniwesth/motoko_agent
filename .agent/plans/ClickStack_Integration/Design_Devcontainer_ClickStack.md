...
| Issue | Plan |
|---|---|
| **Ingestion auth friction** | HyperDX all-in-one requires a first-run user + an ingestion API key pasted into `OTEL_EXPORTER_OTLP_HEADERS`. Local testing on `clickhouse/clickstack-all-in-one:2.21.0` confirmed `http://clickstack:4318` is reachable but returns `401 Unauthorized` without `authorization=<ingestion-key>`. Do not spend time patching the all-in-one internals to bypass auth; that path is likely brittle across image upgrades. Future zero-touch upgrade options: (1) add a local unauthenticated OTel collector in front of ClickStack that forwards to the all-in-one collector with the auth header, or (2) replace all-in-one ingestion with the standalone ClickStack OTel collector, where OSS deployments can omit `OTLP_AUTH_TOKEN` for unauthenticated internal OTLP. Until then, document the one-time key grab + add `OTEL_EXPORTER_OTLP_HEADERS` to both env spots (§5). |
