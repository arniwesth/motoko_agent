...
4. **Rendering synergy (optional, not a dependency):** this plan's `runEventLoop` over the eval channel makes live per-cell rendering cheap — it could emit `eval_cell_start`/`eval_cell_result` events as frames arrive, upgrading [plan 03](./03-eval-tui-card-rendering.md)'s static card into a streaming one. Additive only; plan 03 ships fine against C's single final result. Don't pull plan 03 into this plan's scope.
