# LEG-P1.2b — registration sites, batch B (PLAN-001 (031), line R)
...
`P1.2b` in `PLAN-001-implement-adr-001-abi-8.md` §3 — batch B of the four batches migrating the 45
extension registration sites onto the frozen ABI 8.0. **Batch B:** `motoko-ext-repetition-guard`, `motoko-ext-test-dummy`, `motoko_scratchpad`, `motoko-ext-exa-search`, `motoko-ext-context-mode` — **14 sites**; ints, strings and one config record — the `let`-bound and inline forms with simple data.
...
3. `REVIEW-adr001-v0.6-verdicts-codex.md` **§Q-1** — the 45-row site ledger (your 14 rows).
...
Repetition-guard shares `ExtCtx` helpers between its pure policy and its judge (ADR D2 `:335-337`): those helpers take `PureCtx`.
...
3. **The P0.3 gate**: every batch-B extension's registration-shape result **passes**
...
4. **Masked files** (`scripts/verify_repetition_guard.ail`): each checks on 8.0 in a workspace from committed HEAD, changed only
   where 8.0 requires; one mutgate row each.
5. **mutgate** from a fresh clone/workspace, spec and helpers **committed** under `evidence/P1.2b/`
...
```
ADR-001 (031) P1.2b: batch B onto ABI 8.0 — 14 sites named, config through the channel
```
...
commit: <sha>
files_touched: [<paths>]
items: 14 sites in 5 packages (captures: <n>)
...
- Your 5 packages, your masked files, and `evidence/P1.2b/`. Not `packages/motoko-ext-abi/`
