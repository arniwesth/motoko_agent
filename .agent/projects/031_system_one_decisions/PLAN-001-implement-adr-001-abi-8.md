...
| dst-corpora | `35884636365` PR | **failure** in `pr-corpus`: "the PR corpus target took 106000 ms against a declared ceiling of 80000 ms" | at `2f3ee4d1` the same gate measured **exactly 80000 against 80000** — zero headroom. **Re-measured properly 2026-09-23** (the first comparison used *warm* runs inside a sweep, which this cold gate is not): cold, own packages via `make CI=1 sync_packages`, fresh never-run clones — **`2f3ee4d1` 96 s, `42dc0c18` 87 s**, i.e. the gate was **already failing cold before line R**, and line R made this target **9 s faster**. `src/core/dst_corpus.ail` (both constants) is untouched by line R |
