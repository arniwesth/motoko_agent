#!/usr/bin/env bash
# 031 PLAN-001 P1.3r: P1.1's workspace (evidence/P1.1/p11_core_check.sh, reused
...
#   * the end-to-end runner evidence/P1.3r/cursor_e2e.ail, copied to
#     src/p13r/cursor_e2e.ail (it imports the real runtime and the examples).
...
R=$(pwd)
T=$(mktemp -d "${TMPDIR:-/tmp}/p13rws.XXXXXX"); trap 'rm -rf "$T"' EXIT
...
# P1.3r: the example consumers, exported by the WORKSPACE toml only
mkdir -p "$T/deps/motoko_ext_conformance/examples" "$T/src/p13r"
...
open(p, 'w').write(s)
PY
cp .agent/projects/031_system_one_decisions/evidence/P1.3r/cursor_e2e.ail "$T/src/p13r/cursor_e2e.ail"
...
cd "$T"
ailang lock > lock.log 2>&1 || { tail -5 lock.log; echo "p13r: ailang lock failed in the workspace"; exit 1; }
python3 "$R/tools/ext_registry_gen/generate.py" --config ailang.toml > gen.log 2>&1 \
  || { tail -5 gen.log; echo "p13r: registry generation failed"; exit 1; }
...
if [ "$mode" = e2e ]; then
  ...
  ailang run --caps IO,Process,Clock --entry main src/p13r/cursor_e2e.ail < /dev/null > e2e.log 2>&1; rc=$?
  strip < e2e.log | grep -v -E '^(→|✓ Running)' | sed '/^$/d'
  if [ $rc -ne 0 ]; then echo "p13r: e2e run FAILED (exit $rc)"; exit 1; fi
  if ! grep -q '^END_TO_END finalize=PASS tool_policy=PASS' e2e.log; then echo "p13r: e2e FAILED -- no PASS line"; exit 1; fi
  echo "p13r: e2e ok"; exit 0
fi
fail=0
for m in "$@"; do
  [ -f "$m" ] || { echo "p13r: no such module in the workspace: $m"; fail=1; continue; }
  if [ "$mode" = check ]; then
    out=$(ailang check "$m" 2>&1); rc=$?
    if [ $rc -eq 0 ]; then echo "p13r: check $m ok"
    else
      first=$(echo "$out" | strip | grep -m1 -E 'Error|PAT_|PAR_|LDR0|MOD0|TC0|EFF' | cut -c1-400)
      echo "$out" | strip | tail -8; echo "p13r: check $m FAILED (exit $rc): ${first}"; fail=1
    fi
  else
    ...
    if [ $rc -eq 0 ] && grep -qE '^[0-9]+ tests: [0-9]+ passed, 0 failed' <<<"$out"; then
      echo "p13r: test $m ok -- $line"
    else
      echo "$out" | strip | grep -E 'FAIL|Error|error|tests:' | tail -12; echo "p13r: test $m FAILED (exit $rc; $line)"; fail=1
    fi
  fi
done
exit $fail
