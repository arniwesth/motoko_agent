#!/usr/bin/env bash
# 031 PLAN-001 P1.3r (P1.1's runner, re-pointed): run mutgate.sh (013) over evidence/P1.3r/p13r_mutgate_spec.tsv
# in a THROWAWAY WORKSPACE, never in a checkout anyone is using. Run from a repo root.
#
#   p11_mutgate.sh                    a copy of the cwd's working tree (the
...
# the conformance kit and its examples, the registry generator and this evidence directory.
# Each row's test_cmd is p13r_check.sh, which builds ITS OWN workspace from
...
# evidence/P1.3r/mutgate-result.tsv.
set -uo pipefail
E=.agent/projects/031_system_one_decisions/evidence/P1.3r
M=.agent/projects/013_core_architecture_for_dst/mutgate.sh
[ -f "$M" ] && [ -f "$E/p13r_mutgate_spec.tsv" ] || { echo "p13r_mutgate: run from the repo root" >&2; exit 2; }
T=$(mktemp -d "${TMPDIR:-/tmp}/p13rmut.XXXXXX")
if [ "${1:-}" = --clone-from ]; then
  git clone --shared --quiet "$2" "$T/repo" || { echo "p13r_mutgate: clone failed" >&2; exit 2; }
else
  ...
  cp "$E"/p13r_check.sh "$E"/p13r_mutgate_spec.tsv "$E"/cursor_e2e.ail "$T/repo/$E/"
fi
bash "$M" --spec "$E/p13r_mutgate_spec.tsv" --repo "$T/repo" --out "$T/out"; rc=$?
cp "$T/out/mutgate.tsv" "$E/mutgate-result.tsv"
echo "p13r_mutgate: exit $rc; verdicts in $E/mutgate-result.tsv"
rm -rf "$T"
exit $rc
