#!/usr/bin/env bash
# 031 PLAN-001 P1.1: run mutgate.sh (013) over evidence/P1.1/p11_mutgate_spec.tsv
...
# the conformance kit, the registry generator and this evidence directory.
# Each row's test_cmd is p11_core_check.sh, which builds ITS OWN workspace from
...
# evidence/P1.1/mutgate-result.tsv.
set -uo pipefail
E=.agent/projects/031_system_one_decisions/evidence/P1.1
M=.agent/projects/013_core_architecture_for_dst/mutgate.sh
[ -f "$M" ] && [ -f "$E/p11_mutgate_spec.tsv" ] || { echo "p11_mutgate: run from the repo root" >&2; exit 2; }
T=$(mktemp -d "${TMPDIR:-/tmp}/p11mut.XXXXXX")
if [ "${1:-}" = --clone-from ]; then
  git clone --shared --quiet "$2" "$T/repo" || { echo "p11_mutgate: clone failed" >&2; exit 2; }
else
  ...
  cp "$E"/p11_core_check.sh "$E"/p11_mutgate_spec.tsv "$T/repo/$E/"
fi
bash "$M" --spec "$E/p11_mutgate_spec.tsv" --repo "$T/repo" --out "$T/out"; rc=$?
cp "$T/out/mutgate.tsv" "$E/mutgate-result.tsv"
echo "p11_mutgate: exit $rc; verdicts in $E/mutgate-result.tsv"
rm -rf "$T"
exit $rc
