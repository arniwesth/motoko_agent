...
### D2. Is the repository already public — ANSWERED 2026-09-24: **`origin` is PUBLIC**

Two remotes: `origin` (`arniwesth/motoko_agent`) and `sunholo` (`sunholo-voight-kampff/motoko_agent`).
v0.1 could not verify this from the container (`gh` returned 401 on an invalid token). **Measured
2026-09-24** with a working token, from the 031 orchestrator's session:
`GET /repos/arniwesth/motoko_agent` returns **`"private": false`**, owner `arniwesth`.

So the first branch of this decision holds: **everything already pushed to `origin` is published**, this
tag is **not** the first public exposure, and §5 hygiene is **best-effort over already-published
content** rather than a pre-publication gate. What that changes, for the operator to rule on:

- **G7** was written as "only if D2 = first public exposure". It is not. The question is no longer
  whether to gate the tag on hygiene but whether to run a hygiene pass over what is **already public**
  (branch history included, not only the tip).
- The 031 orchestrator scanned the **five commits it was about to push** for credentials, private keys,
  cloud keys and tailnet/host addresses: clean. That scan covered those commits only, **not** the
  branch's whole history, and not `sunholo`'s visibility, which is still unmeasured.
- `sunholo`'s visibility can be measured the same way when a token with access exists.
...
| **G7 hygiene** | §5 — **D2 answered 2026-09-24: `origin` is already public**, so this is a pass over already-published content, not a pre-tag gate. Operator to rule on scope (tip only, or branch history). | Measured, not applied; the 031 line's five outgoing commits scanned clean (credentials, keys, tailnet/host addresses). |
