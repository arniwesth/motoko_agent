...
SCHEMAS = {
    ...
    # Project 010 projections (generated by tools/code-graph/layout/build_layout.py
    # and tools/code-graph/overlay/build_activity.py; absent until those run).
    "layout": "node_id String, level Int64, x Float64, y Float64, radius Float64, snapshot String",
    "edges_agg": "level Int64, src_agg String, dst_agg String, kind String, weight Int64, exactness String",
    "activity": "seed Int64, event_idx Int64, record_key String, subject_id String, rule_kind String",
}
LAYOUT_TABLES = ("layout", "edges_agg")
...
class QueryFlags:
    ...
    layout_query: bool = False
    activity_query: bool = False
...
def _layout_snapshot_expected(status_rows: list[dict], profile: str) -> str | None:
    """Recompute the layout snapshot key from the current extraction, using the
    builder's own function so the two sides cannot drift on what "same
    snapshot" means. Returns None when the builder is unavailable."""
    sys.path.insert(0, str(TOOL_ROOT / "layout"))
    try:
        from build_layout import compute_snapshot  # type: ignore
    except Exception:
        return None
    return compute_snapshot(status_rows, profile)


def _layout_freshness(status_rows: list[dict], profile: str) -> tuple[bool, str | None, str | None]:
    """(stale, reason, stored_snapshot) for the layout projections."""
    if not (OUT_DIR / "layout.csv").exists():
        return True, "layout.csv missing (run tools/code-graph/layout/build_layout.py)", None
    try:
        rows = run_sql("SELECT DISTINCT snapshot AS s FROM layout").get("data", [])
    except Exception as exc:
        return True, f"layout unreadable: {exc}", None
    stored = sorted({row["s"] for row in rows})
    if len(stored) != 1:
        return True, f"layout carries {len(stored)} snapshot keys (expected exactly 1)", None
    expected = _layout_snapshot_expected(status_rows, profile)
    if expected is None:
        return True, "layout snapshot not verifiable (build_layout.py unavailable)", stored[0]
    if stored[0] != expected:
        return True, ("layout snapshot does not match the current extraction — "
                      "rerun tools/code-graph/layout/build_layout.py"), stored[0]
    return False, None, stored[0]


def _trace_file_count() -> int:
    traces = OUT_DIR / "traces"
    if not traces.is_dir():
        return 0
    return sum(1 for _ in traces.glob("*/*.jsonl"))
...
def status_meta(flags: QueryFlags | None = None) -> dict:
    ...
    meta = {
        ...
        "incomplete": False,
        "incomplete_modules": [],
        # Layout projections have their own freshness key (010 / Q7); activity is
        # keyed to trace files rather than to extractions, so it reports a count
        # instead of an extraction-staleness verdict.
        "layout_snapshot": None,
        "layout_stale": None,
        "layout_stale_reason": None,
        "activity_trace_files": None,
    }
    ...
    if flags.layout_query:
        layout_stale, layout_reason, layout_snapshot = _layout_freshness(rows, meta["profile"])
        meta["layout_stale"] = layout_stale
        meta["layout_stale_reason"] = layout_reason
        meta["layout_snapshot"] = layout_snapshot
    if flags.activity_query:
        meta["activity_trace_files"] = _trace_file_count()
    return meta
...
def flags_for_sql(sql: str) -> QueryFlags:
    """Raw SQL declares nothing, so the projections it touches are detected by
    name — that is what arms the layout staleness banner for ad-hoc queries."""
    lowered = sql.lower()
    return QueryFlags(
        layout_query=any(table in lowered for table in LAYOUT_TABLES),
        activity_query="activity" in lowered,
    )
...
def main() -> int:
    ...
    flags = QueryFlags()
    if ns.cmd == "status":
        flags = QueryFlags(layout_query=True, activity_query=True)
        result = {"data": [], "meta": status_meta(flags)}
    elif ns.cmd == "sql":
        flags = flags_for_sql(ns.sql)
    ...
    if not ns.no_banner and meta.get("layout_stale"):
        print(f"STALE: {meta.get('layout_stale_reason')}", file=sys.stderr)
