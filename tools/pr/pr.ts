...
const GITHUB_DIR = join(REPO_ROOT, ".agent", "github");
const PRS_DIR = join(GITHUB_DIR, "prs");
const STAGING_DIR = join(GITHUB_DIR, "staging");
 ...
 */
function ghEnv(identity: Identity): NodeJS.ProcessEnv {
...
function gh(args: string[], identity: Identity = "operator"): { ok: boolean; stdout: string; stderr: string } {
  const r = spawnSync("gh", args, { encoding: "utf8", env: ghEnv(identity) });
...
function repoSlug(remote: string): string {
  ...
  if (!m) die(`remote '${remote}' does not look like a GitHub remote: ${url}`);
  return m[1];
}

/**
 * D6: the tuple (repo, pr) is authoritative; the alias is a human-facing
 * directory name so colliding PR numbers across the two remotes stay legible.
 */
function alias(remote: string): string {
  return remote;
}
...
function splitDocument(text: string): { frontmatter: string | null; body: string } {
...
function finalPath(remote: string, number: number): string {
  return join(PRS_DIR, `${alias(remote)}-${number}`, "body.md");
}
...
function reportIdentity(identity: Identity): string {
